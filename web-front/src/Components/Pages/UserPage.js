...
import { LoginForm, RegisterForm, ConfirmationCode } from '../Modules/Forms/LoginForms';
...
export function UserPage (props) {
    ...
    const RenderPage = () => {
        switch(screen){
            case "login":
                return <div>
                <LoginForm onEnter={() => navigate("/Store")} ToggleScreen={(screen) => setScreen(screen)} ValidateUser={() => setScreen("confirm")}></LoginForm>
            </div>  
            case "confirm":
                return <ConfirmationCode onValidate={() => setScreen("login")}></ConfirmationCode>
            default: 
